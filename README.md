...
## المساعد الذكي داخل النظام | In-App AI Assistant

لحل مشكلة طلب الإذن من العميل الخارجي في كل عملية (وعدم وصول العميل للبيانات)، أضفنا **مساعد ذكاء اصطناعي مدمجًا داخل لوحة التحكم** نفسها:

- زر عائم بزاوية الشاشة يفتح نافذة محادثة.
- يستخدم نفس أدوات الخادم (بحث/جلب/إنشاء/تعديل/حذف للعقارات والعملاء) لكنه **ينفذها مباشرةً من الخادم** — **بدون أي نافذة سماح/رفض**.
- يوفر للمستخدم سجلًا بالعمليات المنفذة أسفل كل رد.
- يعمل عبر واجهة DeepSeek (متغير `DEEPSEEK_API_KEY` الموجود أصلًا).

### الملفات | Files

```
src/lib/assistant/tool-defs.ts      ← تعريفات الأدوات + المنفّذ
src/lib/assistant/chat.ts           ← حلقة المحادثة (استدعاء الأدوات وتنفيذها)
src/lib/assistant/actions.ts        ← إجراء الخادم (sendAssistantMessage)
src/components/assistant/assistant-widget.tsx ← واجهة المحادثة العائمة
```

### ملاحظة حول الوصول الخارجي

بالنسبة للربط بتطبيقات خارجية (مثل كلود لسطح المكتب)، نافذة «سماح/رفض» سلوك مدمج في التطبيق الخارجي ولا يمكن تعطيلها من الخادم. لتشغيل بدون إذن خارجيًا استخدم:
- **كلود كود (Claude Code)**: قائمة `permissions.tools.alwaysAllow` في `.mcp.json` (موجودة جاهزة).
- **أي عميل يدعم الموافقة التلقائية**: أضف القائمة نفسها في إعداده.
- أو استخدم المساعد المدمج داخل النظام أعلاه.
