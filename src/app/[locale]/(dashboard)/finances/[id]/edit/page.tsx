...
import { getDeal, queryEmployeesForSelect } from "@/lib/finance-actions";
...
export default async function EditDealPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const [deal, employees] = await Promise.all([getDeal(id), queryEmployeesForSelect()]);
  if (!deal) redirect(`/${locale}/finances`);
  return <DealEditPageClient deal={deal} employees={employees} />;
}
