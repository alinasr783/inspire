...
export default async function CompanyClientsPage({
  ...
  let query = admin.from("clients").select("*").eq("is_company_client", true).order("created_at", { ascending: false });
  ...
  const uniqueVals = async (col: string) => {
    const { data } = await admin.from("clients").select(col).eq("is_company_client", true).not(col, "is", null).not(col, "eq", "");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-bold tracking-tight sm:text-2xl">{tNav("companyClients")} <ShortcutsHelp locale={locale} /></h1>
        <div className="-mx-3 flex gap-2 overflow-x-auto px-3 sm:mx-0 sm:px-0">
          {isAdmin && (<Link href="/company-clients/new" className="shrink-0"><Button size="sm"><Plus className="h-4 w-4" />{t("createCompanyClient")}</Button></Link>)}
        </div>
      </div>
