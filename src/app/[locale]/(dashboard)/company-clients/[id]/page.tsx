...
export default async function CompanyClientDetailPage({
  ...
  const canEdit = true;
  const canEditNamePhone = isAdmin;
        ...
        <div className="flex flex-col gap-6 p-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col items-stretch gap-2 lg:items-end">
            ...
            <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
              {canEdit && (
                <Link href={`/company-clients/${clientData.id}/edit`}>
              ...
              {canDelete && <DeleteClientButton clientId={clientData.id} companyMode />}
            </div>
            {!canEditNamePhone && (
              <p className="text-xs text-muted-foreground">{t("namePhoneLocked")}</p>
            )}
          </div>
        </div>
      </Card>
      ...
      {phoneDigits && (
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 backdrop-blur-lg md:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
          <div className="flex items-center gap-2 px-3 py-2">
            ...
            {canEdit && (
              <Link href={`/company-clients/${clientData.id}/edit`} className="flex items-center justify-center rounded-xl border border-border px-3 py-2.5">
                <Pencil className="size-4" />
              </Link>
            )}
            {canDelete && <span><DeleteClientButton clientId={clientData.id} companyMode /></span>}
          </div>
        </div>
      )}
    </div>
