...
import { addUnitDropdownOption } from "@/lib/unit-dropdown-actions";
import { Plus } from "lucide-react";

interface UnitDynamicSelectProps {
  options: string[];
  value: string;
...
export function UnitDynamicSelect({
  options,
  value,
  onChange,
  placeholder,
  category,
}: UnitDynamicSelectProps) {
  ...
  const handleAddNew = async () => {
    if (!search) return;
    const result = await addUnitDropdownOption(category, search);
    if (result.success) {
